 ...
 * Fixed to match the simplified kernel interface
...
void prepareData(const std::vector<int>& row_indices,
                 ...
                 int& row_1_size, int& row_2_size,
                 int& values_1_size, int& values_2_size) {
...
int main(int argc, char** argv) {
    ...
    std::vector<ap_uint<128>> values_indices_1, values_indices_2;
    int row_1_size, row_2_size, values_1_size, values_2_size;

    prepareData(row_indices, col_indices, values, M, N, nnz,
                values_indices_1, values_indices_2,
                row_1_size, row_2_size, values_1_size, values_2_size);
    ...
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_values_indices_1, buffer_values_indices_2, buffer_x}, 0));
    ...
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_y_1, buffer_y_2}, CL_MIGRATE_MEM_OBJECT_HOST));
    OCL_CHECK(err, err = q.finish());
